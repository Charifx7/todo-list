...
<template>
    <div class="flex items-center justify-center bg-gray-100">
        <div class="w-full max-w-3xl bg-white shadow-lg rounded-lg p-8">
            <h1 class="text-4xl font-bold text-center mb-6 text-black">To-do List</h1>
            <div>
                <!-- Input and Add Button -->
                <div class="flex mb-4">
                    <input class="input input-bordered w-full px-4 py-2 text-xl text-white border-gray-300 rounded-lg"
                        placeholder="Insert todo" type="text" v-model="todoText" />
                    <button class="btn btn-primary ml-2 px-6 py-2 text-xl text-white bg-blue-500"
                        @click="addTodo(todoText)">
                        Add
                    </button>
                </div>
                <!-- Tabs for Status -->
                <div class="tabs tabs-bordered mb-6 text-xl text-black">
                ...
                <!-- To-Do Items -->
                <div class="flex items-center justify-between py-2 border-b border-gray-300" v-for="todo in filterTodo"
                    :key="todo.id">
                    <div>
                        <input type="checkbox" :checked="todo.status === 'Done'" @change="changeStatus($event, todo.id)"
                            class="checkbox w-6 h-6 bg-gray-400" />
                    </div>
